...
import { availabilityDoctors, availabilityconsultations } from '../../database/availability.js';

export const getDoctorAvailability = async (req, res) => {
  const { specialityId, date, doctorId } = req.query;

  if (!specialityId || !date) {
    throw generateErrors(400, 'BAD-REQUEST', 'Missing required parameters: specialityId and date');
  }
  ...
  if (doctors.length === 0) {
    throw generateErrors(404, "FORBIDDEN", 'No doctors found for the specified speciality');
  }
